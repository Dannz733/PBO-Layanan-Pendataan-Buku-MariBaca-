...
class Buku {
    ...
    public String getInfo() {
        return judul + " - " + penulis + " (" + (tersedia ? "Tersedia" : "Dipinjam") + ")";
    }
...
public class MariBaca {
    ...
    // Metode utama tambah buku (memilih jenis + input data)
    private static void tambahBuku() {
        ...
        if (jenis == 1) {
            ...
            tambahBuku(judul, penulis, lokasi); // panggil overload fisik
        } else if (jenis == 2) {
            System.out.print("Masukkan ukuran file (MB): ");
            double ukuran = scanner.nextDouble();
            scanner.nextLine();
            tambahBuku(judul, penulis, ukuran); // panggil overload digital
        } else {
